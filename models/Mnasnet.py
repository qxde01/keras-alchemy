...
def conv_bn(x, filters, kernel_size,  strides=1, alpha=1, activation=True):
	...
	x = keras.layers.Conv2D(filters=filters, kernel_size=kernel_size, strides=strides, padding='same',
									use_bias=False, kernel_regularizer=keras.regularizers.l2(l=0.0003),kernel_initializer='he_normal')(x)
	x = keras.layers.BatchNormalization(epsilon=1e-3, momentum=0.999)(x)
	if activation:
...
def depthwiseConv_bn(x, depth_multiplier, kernel_size,  strides=1):
	...
	x = keras.layers.DepthwiseConv2D(kernel_size=kernel_size, strides=strides, depth_multiplier=depth_multiplier,
									padding='same', use_bias=False, kernel_regularizer=keras.regularizers.l2(l=0.0003),kernel_initializer='he_normal')(x)
	x = keras.layers.BatchNormalization(epsilon=1e-3, momentum=0.999)(x)
	x = keras.layers.ReLU(max_value=6)(x)
	return x
...
def MBConv_idskip(x_input, filters, kernel_size,  strides=1, filters_multiplier=1, alpha=1):
	...
	depthwise_conv_filters = _make_divisible(x_input._shape_tuple()[3])
	...
	if strides==1 and x._shape_tuple()[3] == x_input._shape_tuple()[3]:
		return  keras.layers.add([x_input, x])
	else: 
		return x
...
if __name__ == "__main__":
	...
	model.summary()
	keras.utils.plot_model(model, 'MnasNet.png', show_shapes=True)
