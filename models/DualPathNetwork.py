...
__all__ = ['DualPathNetwork', 'DPN92', 'DPN98', 'DPN137', 'DPN107' ]
...
def DualPathNetwork(input_shape=None,
    ...
    # Create model.
    model = keras.models.Model(img_input, x, name='resnext')
...
def DPN92(input_shape=None,
          ...
          pooling=None,
          classes=1000):
    return DualPathNetwork(input_shape, include_top=include_top, weights=weights,
...
def DPN98(input_shape=None,
    ...
    return DualPathNetwork(input_shape, initial_conv_filters=96, depth=[3, 6, 20, 3], filter_increment=[16, 32, 32, 128],
                           cardinality=40, width=4, include_top=include_top, weights=weights,
                           pooling=pooling, classes=classes)
...
def DPN137(input_shape=None,
    ...
    return DualPathNetwork(input_shape, initial_conv_filters=128, depth=[4, 8, 28, 3], filter_increment=[16, 32, 32, 128],
                           cardinality=40, width=4, include_top=include_top, weights=weights,   pooling=pooling, classes=classes)
...
def DPN107(input_shape=None,
    ...
    return DualPathNetwork(input_shape, initial_conv_filters=128, depth=[4, 8, 20, 3], filter_increment=[20, 64, 64, 128],
                           cardinality=50, width=4, include_top=include_top, weights=weights,
                           pooling=pooling, classes=classes)
...
def _initial_conv_block_inception(input, initial_conv_filters, weight_decay=5e-4):
    ...
    x = keras.layers.Conv2D(initial_conv_filters, (7, 7), padding='same', use_bias=False, kernel_initializer='he_normal',
               kernel_regularizer=keras.regularizers.l2(weight_decay), strides=(2, 2))(input)
...
def _bn_relu_conv_block(input, filters, kernel=(3, 3), stride=(1, 1), weight_decay=5e-4):
    ...
    x = keras.layers.Conv2D(filters, kernel, padding='same', use_bias=False, kernel_initializer='he_normal',
               kernel_regularizer=keras.regularizers.l2(weight_decay), strides=stride)(input)
    x = keras.layers.BatchNormalization(axis=channel_axis)(x)
    x = keras.layers.Activation('relu')(x)
    return x
...
def _grouped_convolution_block(input, grouped_channels, cardinality, strides, weight_decay=5e-4):
    ...
    if cardinality == 1:
        ...
        x = keras.layers.Conv2D(grouped_channels, (3, 3), padding='same', use_bias=False, strides=strides,
                   kernel_initializer='he_normal', kernel_regularizer=keras.regularizers.l2(weight_decay))(init)
        x = keras.layers.BatchNormalization(axis=channel_axis)(x)
        x = keras.layers.Activation('relu')(x)
        return x

    for c in range(cardinality):
        x = keras.layers.Lambda(lambda z: z[:, :, :, c * grouped_channels:(c + 1) * grouped_channels]
                   if K.image_data_format() == 'channels_last' else
                   lambda z: z[:, c * grouped_channels:(c + 1) * grouped_channels, :, :])(input)

        x = keras.layers.Conv2D(grouped_channels, (3, 3), padding='same', use_bias=False, strides=strides,
                   kernel_initializer='he_normal', kernel_regularizer=keras.regularizers.l2(weight_decay))(x)

        group_list.append(x)

    group_merge = keras.layers.concatenate(group_list, axis=channel_axis)
    group_merge = keras.layers.BatchNormalization(axis=channel_axis)(group_merge)
    group_merge = keras.layers.Activation('relu')(group_merge)
    return group_merge
...
def _dual_path_block(input, pointwise_filters_a, grouped_conv_filters_b, pointwise_filters_c,
    ...
    init = keras.layers.concatenate(input, axis=channel_axis) if isinstance(input, list) else input
    ...
    if projection:
        ...
        input_residual_path = keras.layers.Lambda(lambda z: z[:, :, :, :pointwise_filters_c]
                                     if K.image_data_format() == 'channels_last' else
                                     z[:, :pointwise_filters_c, :, :])(projection_path)
        input_dense_path = keras.layers.Lambda(lambda z: z[:, :, :, pointwise_filters_c:]
                                  if K.image_data_format() == 'channels_last' else
                                  z[:, pointwise_filters_c:, :, :])(projection_path)
    else:
    ...
    output_residual_path = keras.layers.Lambda(lambda z: z[:, :, :, :pointwise_filters_c]
                                  if K.image_data_format() == 'channels_last' else
                                  z[:, :pointwise_filters_c, :, :])(x)
    output_dense_path = keras.layers.Lambda(lambda z: z[:, :, :, pointwise_filters_c:]
                               if K.image_data_format() == 'channels_last' else
                               z[:, pointwise_filters_c:, :, :])(x)

    residual_path = keras.layers.add([input_residual_path, output_residual_path])
    dense_path = keras.layers.concatenate([input_dense_path, output_dense_path], axis=channel_axis)
...
def _create_dpn(nb_classes, img_input, include_top, initial_conv_filters,
    ...
    x = keras.layers.concatenate(x, axis=channel_axis)

    if include_top:
        avg = keras.layers.GlobalAveragePooling2D()(x)
        max = keras.layers.GlobalMaxPooling2D()(x)
        x = keras.layers.add([avg, max])
        x = keras.layers.Lambda(lambda z: 0.5 * z)(x)
        x = keras.layers.Dense(nb_classes, use_bias=False, kernel_regularizer=keras.regularizers.l2(weight_decay),
        ...
        if pooling == 'avg':
            ...
            a = keras.layers.GlobalMaxPooling2D()(x)
            b = keras.layers.GlobalAveragePooling2D()(x)
...
if __name__ == '__main__':
    ...
    model = DPN92(input_shape=(224, 224, 3),classes=100)
    model.summary()
    keras.utils.plot_model(model, 'DPN92.png', show_shapes=True)
